...
app.get('/InfoCurso', (req, res) => {
	...
	Inscrito.find({curso : req.query.cursoid}, (err,resultInscritos) =>{
		if(err){
			return res.render('ListarCursosInscrito', {
				msg: err
			});
		...
		Curso.find({identificador : req.query.cursoid}, (err,resultCursos) => {
			if(err){
				return res.render('ListarCursosInscrito', {
					msg: err
				});
			...
			CursosDocente.find({curso : req.query.cursoid}, (err,resultDocente) =>{
				if(err){
					return res.render('ListarCursosInscrito', {
						msg: err
					});
				...
				resultInscritos.forEach(function(item){
					Usuario.find({documento : item.estudiante}).exec((err,resultUsuarios) => {
						numeroInscritos ++;
						usuariosCurso.push(resultUsuarios[0]);

						if(numeroInscritos === resultUsuarios.length) {

							Usuario.find({documento : resultDocente[0].docente}).exec((err,resultUsuarioDocente) => {
								docenteCurso.push(resultUsuarioDocente[0]);

								res.render('InfoCurso', {
									curso : resultCursos[0],
									usuarios : usuariosCurso,
									docentes : resultUsuarioDocente
								});
							})
						}
					})
