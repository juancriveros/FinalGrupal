...
io.on('connection', client => {
	...
	client.on('cursocerrado', (curso) =>{
		Curso.find({identificador: curso}).exec((err,resultCursos) => {
			if(err){
				return null
			}

			let texto = 'El curso ' + resultCursos[0].nombre + ' ha sido cerrado por el administrador'
			console.log(texto)
